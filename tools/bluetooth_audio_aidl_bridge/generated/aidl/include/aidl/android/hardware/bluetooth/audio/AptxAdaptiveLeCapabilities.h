...
public:
  ...
  inline bool operator==(const AptxAdaptiveLeCapabilities& _rhs) const {
    return std::tie(pcmBitDepth, samplingFrequencyHz, frameDurationUs, octetsPerFrame, blocksPerSdu) == std::tie(_rhs.pcmBitDepth, _rhs.samplingFrequencyHz, _rhs.frameDurationUs, _rhs.octetsPerFrame, _rhs.blocksPerSdu);
  }
  inline bool operator<(const AptxAdaptiveLeCapabilities& _rhs) const {
    return std::tie(pcmBitDepth, samplingFrequencyHz, frameDurationUs, octetsPerFrame, blocksPerSdu) < std::tie(_rhs.pcmBitDepth, _rhs.samplingFrequencyHz, _rhs.frameDurationUs, _rhs.octetsPerFrame, _rhs.blocksPerSdu);
  }
  ...
  inline std::string toString() const {
    ...
    _aidl_os << "AptxAdaptiveLeCapabilities{";
